...
def _looks_like_personal_help_query(q: str) -> bool:
    patterns = [
        r"\bi need help\b",
        r"\bneed help\b",
        r"\bget help\b",
...
def _looks_like_contact_lookup_query(q: str) -> bool:
    patterns = [
        r"\bwho can i contact\b",
        r"\bwho should i contact\b",
        r"\bwho do i contact\b",
        r"\bwhat number can i call\b",
        r"\bwhat is the number\b",
        r"\bwhat number do i call\b",
        r"\bwhat hotline\b",
        r"\bwhat helpline\b",
        r"\bcontact number\b",
        r"\bphone number\b",
...
def plan_next_actions(
    ...
    # Personal help / distress -> controlled support only
    if is_help or _looks_like_personal_help_query(q):
    ...
    # Informational contact lookup -> retrieval first, AI fallback allowed
    if _looks_like_contact_lookup_query(q):
        if retrieval_match:
            return {
                "actions": ["answer_from_retrieval"],
                "reason": "contact lookup query",
                "risk_level": risk_level,
            }
        return {
            "actions": ["answer_with_llm"],
            "reason": "contact lookup without retrieval match",
            "risk_level": risk_level,
        }
